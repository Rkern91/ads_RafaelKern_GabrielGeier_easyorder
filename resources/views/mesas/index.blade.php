<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('mesas.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-md">Nova mesa</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @forelse($mesas as $mesa)
                        <tr>
                            <td class="px-4 py-2">{{ $mesa->cd_mesa }}</td>
                            <td class="px-4 py-2">{{ $mesa->nm_mesa }}</td>
                            <td class="px-4 py-2 text-right">
                                <a href="{{ route('mesas.edit', $mesa) }}" class="px-3 py-1 rounded bg-blue-600 text-white">Editar</a>
                                <form action="{{ route('mesas.destroy', $mesa) }}" method="post" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="px-3 py-1 rounded bg-red-600 text-white" onclick="return confirm('Excluir esta mesa?')">Excluir</button>
                                </form>
                            </td>
                        </tr>
                    @empty
