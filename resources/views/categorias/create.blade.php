                ...
                <div class="inline-block text-white border border-white/20 rounded-lg p-6" style="background-color:#0f0f0f; width:100%; max-width:600px;">
                    <form method="post" action="{{ route('categorias.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input id="nm_categoria" name="nm_categoria" style="color: black;" value="{{ old('nm_categoria') }}" placeholder="Ex: Bebidas, Comidas, Sobremesas..." class="w-full rounded bg-black text-white placeholder-gray-400 border border-white/20 focus:border-white/40 focus:ring-0 p-2.5">
                            @error('nm_categoria')
                            <div class="text-sm text-red-400 mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="flex justify-center gap-4 pt-6">
                            <a href="{{ route('categorias.index') }}" class="px-5 py-2 rounded bg-white text-black hover:opacity-90 transition" style="background-color: gray; color:white;">
                                Voltar
                            </a>
