                ...
                <div class="inline-block bg-black text-white border border-white/20 rounded-lg p-5"
                    ...
                    <form method="get" class="mb-4" style="margin:20px; text-align:center;">
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-2 w-full">
                            <input name="q" value="{{ $q }}" placeholder="Buscar por #pedido / mesa / nome"
                                   class="rounded bg-black text-white placeholder-gray-400 border border-white/20 focus:border-white/40 focus:ring-0">
                            <input name="mesa" value="{{ $mesa }}" placeholder="Filtrar por mesa"
                                   class="rounded bg-black text-white placeholder-gray-400 border border-white/20 focus:border-white/40 focus:ring-0">
                            <select name="status"
                    ...
                    <div class="overflow-x-auto">
                        <table class="table-auto w-auto text-white bg-black">
                            ...
                            <tr class="border-b border-white/20">
                                <th class="px-4 py-2 text-xs font-semibold uppercase tracking-wide">#</th>
                                <th class="px-4 py-2 text-xs font-semibold uppercase tracking-wide">Mesa</th>
                                <th class="px-4 py-2 text-xs font-semibold uppercase tracking-wide">Valor</th>
                                <th class="px-4 py-2 text-xs font-semibold uppercase tracking-wide">Status</th>
                                <th class="px-4 py-2 text-xs font-semibold uppercase tracking-wide">Data</th>
                            ...
                            @forelse($pedidos as $p)
                                ...
                                <tr class="border-b border-white/10">
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap">
                                        R$ {{ number_format($p->vl_pedido,2,',','.') }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">
                                    ...
                                    <td class="px-4 py-2">
                                        @if(in_array((int)$p->id_status, [0,1], true))
                                            <form method="post" action="{{ route('pedidos.finalizar', $p->cd_pedido) }}"
                                                  class="inline">
                                                @csrf
                                                <button class="px-3 py-1 rounded bg-white text-black hover:opacity-90"
                                                        style="background:#fff;color:#000;margin-right:10px;">
                                                    Finalizar
                                                </button>
                                            </form>
                                        @endif
