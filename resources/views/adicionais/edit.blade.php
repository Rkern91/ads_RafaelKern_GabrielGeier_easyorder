        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-center">
                <div class="inline-block text-white border border-white/20 rounded-lg p-6"
                    ...
                    <form id="formUpdateAdicional" method="post" action="{{ route('adicionais.update', $adicional) }}"
                            ...
                            @if(!empty($adicional->img_b64) && !empty($adicional->img_mime))
                                <div class="flex justify-center">
                                    <img id="preview_atual"
                                         src="data:{{ $adicional->img_mime }};base64,{{ $adicional->img_b64 }}"
                                         alt="Imagem do adicional"
                    ...
                    <div class="flex justify-center gap-4 pt-6">
                        ...
                        <button type="submit" form="formUpdateAdicional"
                                class="px-5 py-2 rounded bg-white text-black hover:opacity-90 transition"
                                style="color: black">Salvar
                        </button>
                        <form method="post" action="{{ route('adicionais.destroy', $adicional) }}"
                              onsubmit="return confirm('Excluir este adicional?')" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
