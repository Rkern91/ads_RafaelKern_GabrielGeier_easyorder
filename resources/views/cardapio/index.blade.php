        ...
        <main class="flex-1">
            <header class="flex items-center justify-between px-4 md:px-6 h-16 border-b border-white/10" style="background-color:#0f0f0f;">
                ...
                <div class="relative">
                    ...
                    <span id="badgeCart" class="absolute -top-2 -right-2 text-xs bg-red-600 text-white rounded-full px-1 hidden">0</span>
                </div>
            </header>
            ...
            <div class="p-4 md:p-6">
                <div class="flex justify-center">
                    <div class="inline-block bg-black text-white border border-white/20 rounded-lg p-5" style="background-color:#0f0f0f; padding:20px; width:100%; max-width:1200px;">
                        ...
                        <form id="formPedido" method="post" action="{{ route('cardapio.confirmar') }}" class="space-y-8">
                            @csrf
                                    ...
                                    @if($lista->isNotEmpty())
                                        <section>
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                @foreach($lista as $p)
                                                    <div class="border border-white/10 rounded p-3" style="background-color:#0f0f0f; margin-bottom:10px; padding:5px;">
                                                        <div class="flex items-center gap-4">
                                                            ...
                                                            <div class="flex items-center gap-2">
                                                                <button type="button" data-type="produto" data-id="{{ $p->cd_produto }}" class="btnMenos px-3 py-1 rounded bg-white text-black" style="color:red; font-weight:bolder;">−</button>
                                                                <span id="qtd-produto-{{ $p->cd_produto }}" class="w-6 text-center" style="text-align:center">0</span>
                                                                <button type="button" data-type="produto" data-id="{{ $p->cd_produto }}" class="btnMais px-3 py-1 rounded bg-white text-black" style="color:green; font-weight:bolder;">+</button>
                                                    ...
                                                    <div class="flex items-center gap-4">
                                                        <div class="shrink-0 rounded overflow-hidden border border-white/10" style="width:120px;height:120px;background:#000;">
                                                            @if(!empty($a->img_b64) && !empty($a->img_mime))
                                                                <img src="data:{{ $a->img_mime }};base64,{{ $a->img_b64 }}" alt="{{ $a->nm_adicional }}" class="w-full h-full object-contain">
                                                            @endif
                                                        </div>

                                                        <div class="flex items-center gap-2">
                                                            <button type="button" data-type="adicional" data-id="{{ $a->cd_adicional }}" class="btnMenos px-3 py-1 rounded bg-white text-black" style="color:red; font-weight:bolder;">−</button>
                                                            <span id="qtd-adicional-{{ $a->cd_adicional }}" class="w-6 text-center" style="text-align:center">0</span>
                                                            <button type="button" data-type="adicional" data-id="{{ $a->cd_adicional }}" class="btnMais px-3 py-1 rounded bg-white text-black" style="color:green; font-weight:bolder;">+</button>
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="obs" name="obs" class="w-full rounded bg-black text-white border border-white/20 focus:border-white/40 focus:ring-0 p-2.5" style="color: black;" rows="3" placeholder="Ex.: sem cebola, ponto da carne, etc."></textarea>
                            </div>

                            <div class="mt-6 flex justify-center">
                                <button id="btnConfirmar" type="submit" class="px-6 py-2 rounded bg-white text-black hover:opacity-90 transition" style="color: black;">Confirmar pedido</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
  ...
  function refreshBadge(){
    ...
    if(s>0){ b.textContent = s; b.classList.remove('hidden'); } else { b.classList.add('hidden'); }
  }
  ...
  if(form){
    form.addEventListener('submit', (e)=>{
      const payload = {
        produtos: Object.entries(state.produtos).filter(([id,q])=>q>0).map(([id,q])=>({id:parseInt(id), qtd:q})),
        adicionais: Object.entries(state.adicionais).filter(([id,q])=>q>0).map(([id,q])=>({id:parseInt(id), qtd:q})),
