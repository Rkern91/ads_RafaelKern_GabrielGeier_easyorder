<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('produtos.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-md">Novo produto</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium">Categoria</th>
                    ...
                    @forelse($produtos as $p)
                        <tr>
                            <td class="px-4 py-2">{{ $p->cd_produto }}</td>
                            <td class="px-4 py-2">{{ $p->nm_produto }}</td>
                            <td class="px-4 py-2">{{ optional($p->categoria)->nm_categoria }}</td>
                            <td class="px-4 py-2">R$ {{ number_format($p->vl_valor, 2, ',', '.') }}</td>
                            <td class="px-4 py-2 text-right">
                                <a href="{{ route('produtos.edit', $p) }}" class="px-3 py-1 rounded bg-blue-600 text-white">Editar</a>
                                <form action="{{ route('produtos.destroy', $p) }}" method="post" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="px-3 py-1 rounded bg-red-600 text-white" onclick="return confirm('Excluir este produto?')">Excluir</button>
                                </form>
                            </td>
                        </tr>
                    @empty
