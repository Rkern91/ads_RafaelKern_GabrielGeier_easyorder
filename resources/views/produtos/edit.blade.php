<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar produto</h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="post" action="{{ route('produtos.update', $produto) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="nm_produto" value="{{ old('nm_produto', $produto->nm_produto) }}" class="w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100">
                        @error('nm_produto') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label class="block mb-1 text-sm">Valor</label>
                        <input type="number" name="vl_valor" value="{{ old('vl_valor', $produto->vl_valor) }}" step="0.01" min="0" class="w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100">
                        @error('vl_valor') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label class="block mb-1 text-sm">Descrição</label>
                        <input name="ds_produto" value="{{ old('ds_produto', $produto->ds_produto) }}" class="w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100">
