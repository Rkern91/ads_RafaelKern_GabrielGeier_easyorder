        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-center">
                <div class="inline-block text-white border border-white/20 rounded-lg p-6" style="background-color:#0f0f0f; width:100%; max-width:720px;">
                    <form id="formUpdateUsuario" method="post" action="{{ route('usuarios.update', $usuario) }}"
                        ...
                        <div class="sm:col-span-2 mb-2">
                            ...
                            <select style="color: black;" name="ds_cargo"
                                ...
                                <option value="adm" @selected(old('ds_cargo', $usuario->ds_cargo ?? '')==='adm')>ADM
                                </option>
                                <option value="caixa" @selected(old('ds_cargo', $usuario->ds_cargo ?? '')==='caixa')>
                                    Caixa
                                </option>
                            </select>
                            @error('ds_cargo')
                    ...
                    <div class="flex justify-center gap-4 pt-6">
                        ...
                        <button type="submit" form="formUpdateUsuario"
                                class="px-5 py-2 rounded bg-white text-black hover:opacity-90 transition"
                                style="color: black">Salvar
                        </button>
                        <form method="post" action="{{ route('usuarios.destroy', $usuario) }}"
                              onsubmit="return confirm('Excluir este usuário?')" class="inline">
                            @csrf
                            @method('DELETE')
